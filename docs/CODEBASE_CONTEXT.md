...
- Implementation is isolated in `/private/tmp/phase3-city-eligibility`, branch `phase3-city-eligibility`, based on Phase 2 Part 2 commit `f43cd42`. Test-first and implementation checkpoints are `68c7eba` and `143f1e2`; no push or merge was performed.
...
- Eligibility is derived on request from the authenticated student's current workspace and student code, the application's persisted `schoolYear`, and canonical Award Registry data. A SCHOOL with no parent follows `DIRECT_CITY` and has no Award prerequisite. A SCHOOL whose immediate parent is `UNIVERSITY_SYSTEM` follows `UDN_PREREQUISITE` and requires both a School award and a UDN award.
- Award lookups require exact `studentCode`, `institutionWorkspaceId`, issuer workspace, award level, application school year, and `AwardDecision.status = CONFIRMED`. The School award issuer and recipient institution must both be the student's School. The UDN award issuer must be the immediate parent and its recipient institution must be the student's School. Matching does not require `matchedUserId`; Draft/Archived decisions, old school years, other institutions, preview rows, names, and global student-code matches do not count.
- The DTO returns the application id/year, route, eligibility boolean, required/satisfied state for each Award prerequisite, and ordered blocking reason codes. Direct City reports both prerequisites as not required and satisfied. A missing student code blocks only the UDN route. Unsupported workspace configuration returns a conflict error rather than inferring a route.
- No schema or migration, frontend, submit gate, target-level behavior, precheck/rules, review engine, or Award Registry mutation was added. Phase 3 Part 2 remains responsible for applying an eligibility gate during submit.
- Verification: focused eligibility tests passed (3 files, 23 tests); `npx prisma validate`, `npx prisma generate`, build, and lint passed. Lint reports 33 existing `no-explicit-any` warnings. Full `npm test` ran 89 files: 84 passed, 492 tests passed, 22 skipped, and 5 integration suites failed before assertions because PostgreSQL rejected the configured `postgres` credentials. The blocked suites were approved evidence names, non-AI application flow, Award roster ingestion, workspace isolation, and Award Decision registry.
